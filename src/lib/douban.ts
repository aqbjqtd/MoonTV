 ...
 * 代理健康状态缓存
 */
const proxyHealthCache = new Map<string, { healthy: boolean; lastCheck: number }>();
...
async function checkProxyHealth(proxy: string): Promise<boolean> {
  ...
  try {
    const response = await fetch(`${proxy}/rexxar/api/v2/subject/recent_hot/movie?start=0&limit=1`, {
      method: 'HEAD',
 ...
 */
export async function fetchDoubanData<T>(url: string, maxRetries: number = 3): Promise<T> {
  let lastError: Error | null = null;

  // 尝试直连豆瓣API
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      console.log(`[豆瓣API] 尝试直连 (${attempt + 1}/${maxRetries + 1}): ${url}`);
      ...
      console.warn(`[豆瓣API] 直连失败 (${attempt + 1}/${maxRetries + 1}):`, (error as Error).message);
